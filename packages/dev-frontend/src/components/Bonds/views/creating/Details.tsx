  ...
  return (
    <>
      <Heading as="h2" sx={{ pt: 1, pb: 3, px: 2 }}>
        <Flex sx={{ justifyContent: "center" }}>Bond IBRL</Flex>
        <Close
          ...
          events={[
            {
              ...
              label: (
                <>
                  <Label subLabel="0 iBRL" description={l.BOND_CREATED.description}>
                    {l.BOND_CREATED.term}
                  </Label>
                  <SubLabel>0 iBRL</SubLabel>
                </>
              ),
              isEndOfLife: true
            },
                ...
                <>
                  ...
                  <SubLabel>
                    <InfiniteEstimate estimate={breakEvenAccrual}>
                      {breakEvenAccrual.prettify(2)} iBRL
                    </InfiniteEstimate>
                  </SubLabel>
                </>
              )
      ...
      <HorizontalSlider
        ...
        description={`The market price of iBRL impacts how long it will take to rebond and break even. The actual times may be overestimated as the simulator is based on the current iBRL accrual rate, not taking into account potential rate adjustments.`}
      ...
      {!protocolInfo.hasMarketPremium && (
        <Warning>
          When the iBRL market price is less than 3% above the floor price, it's not profitable to
          bond. Buying iBRL from the market currently generates a higher return than bonding.{" "}
      ...
      {!isInfiniteBondApproved && (
        <ActionDescription>
          <Text>You are approving IBRL for bonding</Text>
        </ActionDescription>
      )}

      {statuses.APPROVE === "FAILED" && (
        <Warning>Failed to approve spend of IBRL. Please try again.</Warning>
      )}
      ...
      {isInfiniteBondApproved && (
        <ActionDescription>
          You are bonding <Text sx={{ fontWeight: "bold" }}>{deposit.prettify(2)} IBRL</Text>
        </ActionDescription>
      )}

      {!isDepositEnough && <ErrorDescription>The minimum bond amount is 100 IBRL.</ErrorDescription>}
      {doesDepositExceedBalance && (
        <ErrorDescription>
          Amount exceeds your balance by <Amount>{deposit.sub(lusdBalance).prettify(2)} IBRL</Amount>
        </ErrorDescription>
      )}

      <Flex pb={2} sx={{ fontSize: "15.5px", justifyContent: "center", fontStyle: "italic" }}>
        You can cancel your bond at any time to recover your deposited IBRL
      </Flex>
